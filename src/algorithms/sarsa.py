"""SARSA (on-policy TD(0)) for tabular environments.

Update rule:
    Q(s,a) ← Q(s,a) + α [r + γ Q(s',a') − Q(s,a)]

α and ε are annealed linearly over episodes using a step counter.
"""
...
@dataclass
class SarsaConfig:
    """Hyperparameters for a single SARSA run."""
...
def run_sarsa(
    ...
    """Run SARSA on *env* and return (Q-table, trace).
    ...
    for ep in range(1, n_episodes + 1):
        ...
        while not done:
            ...
            # On-policy: pick next action before update
            next_action = (
                epsilon_greedy_action(Q, next_state, epsilon, rng) if not done else 0
            )

            # SARSA update
            td_target = float(reward) + (
                0.0 if done else config.gamma * Q[next_state, next_action]
            )
            ...
            # Refresh schedules each env step
            alpha = linear_schedule(
                step, config.alpha_start, config.alpha_end, config.alpha_decay_steps
            )
            epsilon = linear_schedule(
        ...
        if ep % config.convergence_window == 0:
            ...
            if logger and ep % log_interval == 0:
                logger.info(
                    "  SARSA ep %d/%d  window_mean=%.4f  α=%.4f  ε=%.4f",
                    ep,
                    n_episodes,
                    wm,
                    alpha,
            ...
            if check_convergence(
                ...
                if logger:
                    logger.info(
                        "  SARSA converged at episode %d (window_mean plateau)", ep
                    )
                trace.append(record)
                break
        elif logger and ep % log_interval == 0:
            logger.info(
                "  SARSA ep %d/%d  ep_return=%.1f  α=%.4f  ε=%.4f",
                ep,
                n_episodes,
                ep_return,
                alpha,
